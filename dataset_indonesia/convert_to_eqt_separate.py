...
def load_npz_file(filepath):
    ...
    try:
        ...
        waveform = data['waveform']  # shape should be (30085, 3)
        p_idx = data['p_idx']
        s_idx = data['s_idx']
        station = str(data['station'])
        network = str(data['network'])
        location = str(data['location']) if 'location' in data else ''
...
def create_eqt_format_separate(npz_dir, train_list_path, valid_list_path):
    ...
    summary = {
        ...
        'train_hdf5_file': 'indonesia_train_dataset.hdf5',
        'train_csv_file': 'indonesia_train_dataset.csv',
        'valid_hdf5_file': 'indonesia_valid_dataset.hdf5',
        'valid_csv_file': 'indonesia_valid_dataset.csv',
...
def process_dataset(npz_dir, file_list, dataset_type):
    ...
    for i, filename in enumerate(file_list):
        ...
        metadata_entry = {
            ...
            'p_arrival_sample': int(npz_data['p_idx']),
            'p_status': 'manual',
            'p_weight': 1.0,
            'p_travel_sec': int(npz_data['p_idx']) / 100.0,  # Convert sample to seconds (100 Hz)
            's_arrival_sample': int(npz_data['s_idx']),
            's_status': 'manual', 
            's_weight': 1.0,
            's_travel_sec': int(npz_data['s_idx']) / 100.0,  # Convert sample to seconds
...
if __name__ == "__main__":
    # Configuration
    npz_directory = "npz_padded"
    train_list_file = "padded_train_list.csv"
    valid_list_file = "padded_valid_list.csv"
